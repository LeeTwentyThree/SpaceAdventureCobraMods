...
public class RuntimeAdditiveAnimation : MonoBehaviour
{
    ...
    [FormerlySerializedAs("fadeDuration")] public float transitionDuration;
    ...
    void LateUpdate()
    {
        ...
        foreach (var bone in AnimatedBones)
        {
            ...
            bone.Bone.localRotation = Quaternion.Slerp(bone.Bone.localRotation, rot, weight);
            bone.Bone.localPosition = Vector3.Lerp(bone.Bone.localPosition, pos, weight);
        }
    ...
    {
        if (_playbackTime < transitionDuration)
        {
            return _playbackTime / transitionDuration;
        }

        if (_playbackTime > ClipLength - transitionDuration)
        {
            return (ClipLength - _playbackTime) / transitionDuration;
        }
